...
export class EditDocumentComponent implements OnInit, OnChanges {
  ...
  @Input() type: string = "attachment"; // "空"仅显示附件列表无上传、image仅图片类型、attachment文档类型、total图片及文档类型
  ...
  async queryDomain() {
    ...
    if (company.get("configQiniu") && company.get("configQiniu").domain) {
      ...
      this.qiuniuDomain = "https://cloud.file.futurestack.cn/";
    }
      ...
      if (file && file.url) {
        ...
        if (file.url.indexOf("undefined") >= 0) {
          ...
          this.files =
            (this.qiuniuDomain
              ? this.qiuniuDomain
              : "https://cloud.file.futurestack.cn/") +
      ...
      case "attachment":
        ...
        case "total":
          this.mime_types = [
            {
              title: "Total files",
              extensions: "jpg,gif,png,pdf,doc,docx,xls,md,txt,json"
            }, // 限定pdf,doc,docx,xls,md,txt后缀上传
          ];
          break;
      case "video":
