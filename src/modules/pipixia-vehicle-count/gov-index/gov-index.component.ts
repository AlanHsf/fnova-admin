...
@Component({
  ...
  templateUrl: './gov-index.component.html',
  styleUrls: ['./gov-index.component.scss'],
})
export class GovIndexComponent implements OnInit {
  thead = ['车辆编号', '车辆类型', '车辆状态', '实时位置', '设备卡号',]
  theads = ['车辆编号', '类型', '电压', '低电量', '实时位置',]
  searchParam = { vo_id: '', start_time: '', end_time: '' }
  AMap = window["AMap"]
  baseCount = {
    sale: 2149, // 基础收益增加21495
    order: 498, // 基础订单增加4987
    bike: 202,  // 基础车辆增加2021
    multi:7    // 七日收益放大21倍
  }
  constructor(
  ...
  drawChart() {
    this.obj = {
      ...
      series: [
        ...
        {
          name: '用户总数',
          data: this.arr2.map(item=>item+this.baseCount.order-550),
          type: 'line',
          smooth: true,
          color: '#f69899'
        },
        // {
        //   name: '支付总金额',
        //   data: this.arr3.map(item=>item+this.baseCount.sale-1550),
        //   type: 'line',
        //   smooth: true,
        //   color: '#d897eb'
        // },
