...
@Component({
  selector: 'app-import-school',
  templateUrl: './import-school.component.html',
  styleUrls: ['./import-school.component.scss']
})
    ...
    this.activRoute.paramMap.subscribe(async (params) => {
      if (!this.department) {
        ...
        if (departInfo) {
          ...
          this.classExcTpl = this.companyInfo.get('config')?.schoolExcTpl
          console.log(this.classExcTpl, this.companyInfo, this.classExcTpl);
          if (!this.classExcTpl) {
            this.classExcTpl = company.get("config")?.schoolExcTpl
          }
        }
      }
    });
      ...
      for (let index = 0; index < columns.length; index++) {
        ...
        for (let dIndex = 0; dIndex < datas.length; dIndex++) {
          ...
          if (column.unique && !repeatRef) {
            console.log('unique');
            let TargetClass = new Parse.Query("Category");
  ...
  async saveLine() {
    ...
    for (let j = 0; j < this.rowData.length; j++) {
      ...
      for (let i = 0; i < this.columnDefs.length; i++) {
        ...
        categoryInfo.set("showType", "school_area");
