...
@Component({
  selector: 'app-edit-filemanager',
  templateUrl: './edit-filemanager.component.html',
  styleUrls: ['./edit-filemanager.component.scss']
})
export class EditFileManagerComponent implements OnInit {
  local: any
  /** 标签组件 <Tag> EditImage
     ...
     @params mode <string> 选填 comp组件（默认） callback函数
  ...
  selectArray():Array<ParseObject>{
    ...
    // console.log(this.selectMap, Object.values(this.selectMap));
  ...
  async loadAttachment(cate?,mime?){
    console.log(cate);
    ...
    // query.skip(this.attachSize * this.attachPageIndex);
    query.find().then(data=>{
