...
export class GradeCollectComponent implements OnInit {
	...
	async ngOnInit() {
		if (localStorage.getItem('department')) {
			...
			await Department.get(this.department).then(res => {
				if (res && res.get('school').length > 0) {
					console.log(res.get('school'))
  ...
  (select
   ...
   group by "Credit"."uid") as "getCredit" on "getCredit"."uid" = "PInfo"."uid"
	...
	toDetail(uid) {
		this.router.navigate(['/masterol/grade-detail', { uid: uid }])
	}
	...
	search() {
		...
		console.log(this.school, this.student, this.major)
		let sql
		if (localStorage.getItem('department')) {
			let department = localStorage.getItem('department')
			if (!this.isSchool) {
