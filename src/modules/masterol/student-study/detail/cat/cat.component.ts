...
export class CatComponent implements OnInit {
    ...
    async getLesson() {
        ...
        let lesson = await queryLesson.get(this.lessonId);
        console.log(this.lessonId, lesson);
        if (lesson && lesson.id) {
            this.lesson = lesson;
        }
    }
    ...
    async getArticle() {
        ...
        this.open(index).then(res => {
            ...
            console.log(firstVideoUrl, this.article[0].sections[0])
            this.playVideo(firstVideoUrl, this.article[0].sections[0], this.df, 0);
        })
        return
    }
    ...
    async playVideo(videoUrl, section, df, index) {
        ...
        if (this.lessonType == 'score') {
            videojs(vpid).currentTime = function currentTime(seconds) {
                if (typeof seconds !== 'undefined') {
                    ...
                    let goTime = seconds;
                    if (seconds <= that.recordTime || that.lesson.get("isDrag")) {// 拖动时间小于当前时间或课程设置为可拖动
                        goTime = seconds;
                    } else {// 拖动时间大于当前时间
