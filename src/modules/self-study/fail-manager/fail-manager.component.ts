...
// 不及格成绩列表
@Component({
  selector: 'app-fail-manager',
  templateUrl: './fail-manager.component.html',
  styleUrls: ['./fail-manager.component.scss'],
  providers: [DatePipe]
})
export class FailManagerComponent implements OnInit {
  ...
  async getProfiles(skip?, inputVal?, filter?) {
    ...
    let fromSql = ` from 
      (select "objectId","name","studentID","idcard","department","center","school","batch","identyType","education","SchoolMajor","createdAt" from "Profile" where "company" = '1ErpDVc1u6' and "isDeleted" is not true and "isCross" is true) as "pro"
      left join 
      ...
      join (select "lesson","major","majorType","examType" from "ExamPlan") as "plan" on ("major"."objectId" = "plan"."major" and "major"."types" = "plan"."majorType")
      join 
        (select "objectId","title" from "Lesson" where "company" = '1ErpDVc1u6') as "lesson"
          on "lesson"."objectId" = "plan"."lesson" 
      left join
        (select "objectId","major","profile","lesson","grade","type" from "CreditGrade" where "company" = '1ErpDVc1u6' ) as "grade"
    ...
    if (this.centerId) {
      fromSql = ` from 
      (select "objectId","name","studentID","idcard","department","center","school","batch","identyType","education","SchoolMajor","createdAt" from "Profile" where "departments" @> '[{ "objectId": "${this.centerId}"}]' and "company" = '1ErpDVc1u6' and "isDeleted" is not true and "isCross" is true) as "pro"
      left join 
        (select "objectId","name" as "majorName","type","types" from "SchoolMajor" where "company" = '1ErpDVc1u6') as "major"
          on ("pro"."SchoolMajor" = "major"."objectId" and "pro"."education" = "major"."type" and "pro"."identyType" = "major"."types")
      join (select "lesson","major","majorType","examType" from "ExamPlan") as "plan" on ("major"."objectId" = "plan"."major" and "major"."types" = "plan"."majorType")
     ...
     `
      ...
      fromSql = ` from 
      (select "objectId","name","studentID","idcard","department","center","school","batch","identyType","education","SchoolMajor","createdAt" from "Profile" where "departments" @> '[{ "objectId": "${this.department}"}]' and "company" = '1ErpDVc1u6' and "isDeleted" is not true and "isCross" is true) as "pro"
      left join 
        (select "objectId","name" as "majorName","type","types" from "SchoolMajor" where "company" = '1ErpDVc1u6') as "major"
          on ("pro"."SchoolMajor" = "major"."objectId" and "pro"."education" = "major"."type" and "pro"."identyType" = "major"."types")
      join (select "lesson","major","majorType","examType" from "ExamPlan") as "plan" on ("major"."objectId" = "plan"."major" and "major"."types" = "plan"."majorType")
    ...
    let whereSql = ` where "grade"."grade" is null or "grade"."grade" < '60' `
