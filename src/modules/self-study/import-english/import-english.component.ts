...
@Component({
  selector: 'app-import-english',
  templateUrl: './import-english.component.html',
  styleUrls: ['./import-english.component.scss']
})
export class ImportEnglishComponent implements OnInit {
  public api: GridApi;
    ...
    this.activRoute.paramMap.subscribe(async (params) => {
      if (!this.department) {
        ...
        if (centerInfo) {
          ...
          // 学位报名导入文件与准考证号导入文件一致, 所以可以直接复用studentIdExcTpl
        ...
        for (let dIndex = 0; dIndex < columns.length; dIndex++) {
          ...
          if (column.require && (!field || field.trim() == '')) {
            ...
            requireRef = this.modal.confirm({
              nzTitle: 'excel上传错误',
              nzContent: `必填项 ${column.field}  ${dIndex + 1}列为空，请处理后上传`,
              nzCancelText: null,
            });
            resolve(false)
          }

          if(column.field == '是否报名培训(必填)'){
            if (field != '1' && field != '0') {
              this.isImport = false;
              requireRef = this.modal.confirm({
                nzTitle: 'excel上传错误',
                nzContent: `必填项 ${column.field}  ${dIndex + 1}列参数错误，请处理后上传`,
                nzCancelText: null,
              });
              resolve(false)
            }
          ...
          if(column.field == '报考语种(必填)'){
            console.log(field)
            if (field != '英语' && field != '日语' && field != '俄语' && field != '法语' && field != '德语') {
              this.isImport = false;
              requireRef = this.modal.confirm({
                nzTitle: 'excel上传错误',
                nzContent: `必填项 ${column.field}  ${dIndex + 1}列参数错误，请处理后上传`,
          ...
          if (column.key == 'idcard' && item[column.field]) {
            ...
            if (!profileInfo) {
              ...
              resolve(false)
            } else if (profileInfo && profileInfo.id) {
              console.log(profileInfo)
              let grade = new Parse.Query("GradeInfo")
              grade.equalTo("profile", profileInfo.id)
              let gradeInfo = await grade.first();
              if (gradeInfo && gradeInfo.id) {
                console.log(profileInfo)
                this.isImport = false;
                requireRef = this.modal.confirm({
                  nzTitle: 'excel上传错误',
                  nzContent: `${column.field}: ${item["身份证号(必填)"].trim()} ，该学生已报名学位考试，请处理后上传`,
                  nzCancelText: null,
                });
                resolve(false)
              }
              console.log(profileInfo.get('education'), profileInfo.get('education'))
              if (profileInfo.get("education") && profileInfo.get("education") == '专科') {
                console.log(profileInfo)
                this.isImport = false;
                requireRef = this.modal.confirm({
                  nzTitle: 'excel上传错误',
                  nzContent: `${column.field}: ${item["身份证号(必填)"].trim()} ，该学生层次为专科，请处理后上传`,
                  nzCancelText: null,
                });
                resolve(false)
              }

              if (profileInfo.get("SchoolMajor").get("name") && profileInfo.get("SchoolMajor").get("name").indexOf("英语") != -1 && item["报考语种(必填)"].trim() == '英语') {
                console.log(profileInfo)
                this.isImport = false;
                requireRef = this.modal.confirm({
                  nzTitle: 'excel上传错误',
                  nzContent: `${column.field}: ${item["身份证号(必填)"].trim()} ，该学生专业为英语，不能报考英语考试，请处理后上传`,
                  nzCancelText: null,
                });
                resolve(false)
              }else if (profileInfo.get("SchoolMajor").get("name") && profileInfo.get("SchoolMajor").get("name").indexOf("英语") == -1 && item["报考语种(必填)"].trim() != '英语') {
                console.log(profileInfo)
                this.isImport = false;
                requireRef = this.modal.confirm({
                  nzTitle: 'excel上传错误',
                  nzContent: `${column.field}: ${item["身份证号(必填)"].trim()} ，该学生专业不为英语，只能报考英语考试，请处理后上传`,
                  nzCancelText: null,
                });
                resolve(false)
              }
  ...
  async saveLine() {
    ...
    for (let j = 0; j < this.rowData.length; j++) {
      ...
      let apply = Parse.Object.extend("GradeInfo");
      let applyInfo = new apply();
      applyInfo.set("profile", {
        __type: "Pointer",
        className: "Profile",
        objectId: profileInfo.id
      });
      applyInfo.set("company", {
        __type: "Pointer",
        className: "Company",
        objectId: "1ErpDVc1u6"
      });
      applyInfo.set("school", {
        __type: "Pointer",
        className: "Department",
        objectId: profileInfo.get("department").id
      });
      applyInfo.set("center", {
        __type: "Pointer",
        className: "Department",
      ...
      applyInfo.set("batch", profileInfo.get("batch"))
      applyInfo.set("langCode", row["报考语种(必填)"])
      applyInfo.set("signDate", new Date(row["报名时间(必填)"]))
      applyInfo.set("isTrain", row["是否报名培训(必填)"])

      count += 1;

      console.log(applyInfo)
      await applyInfo.save().then((res) => {
        if (res && res.id) {
          this.count++;
          console.log(res)
        } else {
