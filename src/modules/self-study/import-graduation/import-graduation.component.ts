...
@Component({
  selector: 'app-import-graduation',
  templateUrl: './import-graduation.component.html',
  styleUrls: ['./import-graduation.component.scss']
})
export class ImportGraduationComponent implements OnInit {
  public api: GridApi;
    ...
    this.activRoute.paramMap.subscribe(async (params) => {
      if (!this.department) {
        ...
        if (centerInfo) {
          ...
          this.classExcTpl = this.companyInfo.get('config')?.graduationExcTpl
          console.log(this.classExcTpl, this.companyInfo);
          if (!this.classExcTpl) {
            this.classExcTpl = company.get("config")?.graduationExcTpl
          }
          this.departs = await this.getDeparts();
        } else {
  ...
  async saveLine() {
    ...
    for (let j = 0; j < this.rowData.length; j++) {
      ...
      let appEdu = Parse.Object.extend("SchoolAppEdu");
      let appEduInfo = new appEdu();
      for (let i = 0; i < this.columnDefs.length; i++) {
        ...
        appEduInfo.set("company", {
          __type: "Pointer",
          className: "Company",
          objectId: "1ErpDVc1u6"
        });
        appEduInfo.set("department", {
          __type: "Pointer",
          className: "Department",
          objectId: this.department
        });
        appEduInfo.set("center", {
          __type: "Pointer",
          className: "Department",
          objectId: profileInfo.get("center").id
        });
        appEduInfo.set("major", {
          __type: "Pointer",
          className: "SchoolMajor",
          objectId: profileInfo.get("SchoolMajor").id
        });
        appEduInfo.set("departments",
          [{
            __type: "Pointer",
            className: "Department",
            objectId: this.department
      ...
      console.log(appEduInfo)
      await appEduInfo.save().then((res) => {
        if (res && res.id) {
          this.count++;
          console.log(res)
        } else {
