...
@Component({
  selector: 'app-paper-report',
  templateUrl: './paper-report.component.html',
  styleUrls: ['./paper-report.component.scss']
})
export class PaperReportComponent implements OnInit {
  ...
  async getProfiles(skip?, inputVal?, filter?) {
    ...
    let fromSql = ` from 
      (select "objectId","title","remark","count","batch","school","center","excelFile","document","isDownload","isReport","departments","createdAt" from "PaperSubmit" where "company" = '1ErpDVc1u6' and "type" = 'paper') as "submit"
    ...
    if (this.searchType.type == 'Boolean') {
      if (this.inputValue == "已上报" || this.inputValue == "已下载") {
        whereSql += ` and "${this.searchType.schemaAlia}"."${this.searchType.value}" is true `
      } else if (this.inputValue == "未上报" || this.inputValue == "未下载") {
        ...
        if (that.data.length) {
          ...
          if (repeatIdcard.length > 0) {
            ...
            if (!flag) {
              ...
              let baseurl = localStorage.getItem("NOVA_SERVERURL") ? localStorage.getItem("NOVA_SERVERURL") + "api/novaql/select" : "https://server.fmode.cn/api/novaql/select";
              let paperSql = `select "pro"."objectId" as "proObjectId", "paper"."objectId" as "paperObjectId" from 
              (select "objectId" from "Profile" where "company" = '1ErpDVc1u6' and "department" = '${that.depaId}' and "center" = '${that.centerId}' and "idcard" in (${inSql})) as "pro" 
              join (select "objectId","profile" from "SchoolPaper" where "company" = '1ErpDVc1u6') as paper on "paper"."profile" = "pro"."objectId"`
              that.http.post(baseurl, { sql: paperSql }).subscribe(async (res: any) => {
                console.log(res);
                if (res.code == 200) {
                  if (res.data && res.data.length) {
                    for (let i = 0; i < res.data.length; i++) {
                      paperIds.push(res.data[i].paperObjectId)
                    }
      ...
      for (let index = 0; index < datas.length; index++) {
        ...
        for (let dIndex = 0; dIndex < columns.length; dIndex++) {
          ...
          if (column.key == 'idcard' && item[column.field]) {
            ...
            join (select "objectId" from "SchoolMajor" where "name" = '${item["专业名称(必填)"].trim()}' and "majorCode" = '${item["专业代码(必填)"].trim()}') as "major" on "pro"."SchoolMajor" = "major"."objectId" 
            left join (select "title","profile","isReport" from "SchoolPaper" where "company" = '1ErpDVc1u6') as "paper" on "paper"."profile" = "pro"."objectId" 
            left join (select "objectId" as "subObjectId","title" as "subTitle","isReport" as "subReport" from "PaperSubmit" where "company" = '1ErpDVc1u6' and "type" = 'paper' and "school" = '${this.depaId}' and "center" = '${this.centerId}') as "sub" on "sub"."subObjectId" = "paper"."isReport"`
            console.log(sql)
                ...
                if (res.data && res.data.length) {
                  if (res.data[0].isReport && res.data[0].subTitle) {
                    let msg = {
                      ...
                      name: item["姓名(必填)"],
                      idcard: item["身份证号(必填)"],
                      res: `该学生论文数据已存在标题为 [${res.data[0].subTitle}] 的上报计划中!`
                    }
                    this.errData.push(msg)
                    errorNum++
                  } else if (!res.data[0].title) {
                    let msg = {
                      num: `第 ${index + 1} 行`,
                      name: item["姓名(必填)"],
                      idcard: item["身份证号(必填)"],
