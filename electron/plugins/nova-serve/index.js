...
module.exports = options => {
	...
	return async window_ => {
		if(window_.loadURL){ // 支持window渲染
			await window_.loadURL(`${options.scheme}://-`);
		}
		if(window_.webContents&&window_.webContents.loadURL){ // 支持view渲染
			await window_.webContents.loadURL(`${options.scheme}://-`);
