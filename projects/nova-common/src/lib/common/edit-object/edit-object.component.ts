...
export class EditObjectComponent implements OnInit {
	...
	ngOnInit() {
		this.activatedRoute.paramMap.subscribe(async (parms) => {
			...
			if (parms && parms.get("rid")) {
				...
				if (route && route.id) {
					if (route.get("editFields") && route.get("editFields").length > 0) {
	...
	initSchemaAndFields(isNew?) {
		...
		if (className) {
			...
			// schame.editTabs 和this.editTabs 不存在的时候
			if (
				this.Schema.editTabs &&
				(!this.editTabs || this.editTabs.length == 0)
			) {
	...
	async save() {
		...
		fieldsArray.filter((fieldItem) => {
			...
			if (fieldItem.type == "String") {
				if (this.object[key]) {
          console.log(key)
					this.object[key] = this.object[key] ? this.object[key].trim() : "";
				}
			}
			// 需要正则匹配
			if (fieldItem.match) {
		...
		if (this.className == "_Role") {
			...
			// roleACL.setRoleReadAccess(`${this.appServ.company}_admin`, true);
			// roleACL.setRoleWriteAccess(`${this.appServ.company}_admin`, true);
