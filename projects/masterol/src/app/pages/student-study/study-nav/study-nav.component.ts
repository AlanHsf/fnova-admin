...
export class StudyNavComponent implements OnInit {
  ...
  tabs = [
    ...
    // {
    //   name: "我的学籍",
    //   icon: "../../../assets/img/masterol/img/xj.png"
    // },
  ]
@Input() LessonId: any;
  ...
  ngOnInit() {
    ...
    if (this.pathUrl == "/masterol/student-study/test" && this.pathParam == 'type=1') {
      this.currentNav = 1
      // console.log(this.currentNav)


    } else if (this.pathUrl == "/masterol/student-study/test" && this.pathParam == 'type=2') {
      this.currentNav = 2
      // console.log(this.currentNav)
  ...
  toTest(pageType,types?) {
    // if (!types) {
      if(pageType == 'detail') {
        this.router.navigate(["masterol/student-study/detail"])
        this.currentNav = 0
        // console.log(123)
      }
      if(pageType == 'test') {
  ...
  toHomeNav(i){
    this.router.navigate(['masterol/student-center',{i:i}])
  }
}
