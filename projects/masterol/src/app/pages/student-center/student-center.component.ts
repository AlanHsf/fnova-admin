...
export class StudentCenterComponent implements OnInit {
  ...
  async goArticle(id) {
    ...
    company.equalTo("objectId", this.schoolname);
    let Company: any = await company.first();
    let website = Company.get('website')
    window.open(`http://${website}/article/${id}`, "_blank");
  }
          ...
          if (res.get('lessons') && res.get('lessons').length > 0) {
            ...
            res.get('lessons').forEach(lesson => {
              if (lesson.get('status')) {
                if (lesson.get("endTime") && this.myDate < lesson.get("endTime")) {
    ...
    query.first().then(res => {
      if (res && res.id) {
        ...
        if (res.get("lessons")) {
          ...
          this.lowesterCredits = res.get("minScore");
  ...
  // 打印合格证
  getPrintCertificate(){
    if(!this.SchoolPlan.get('minScore')){
      this.lowesterCredits = 0      
    }
    if(this.credits >= this.lowesterCredits){
      this.router.navigate([
        `masterol/print-certificate`
      ]);
    }else{
      this.message.warning("学分低于应获总学分，未达到合格要求！");
    }
  }
  totalLesson: any = [];
  showSemester: boolean = false; // 是否显示开课学期
  loading: boolean = true
  tkLoading: boolean = true
  myDate = new Date();
  ...
  getLesson(lessons) {
    ...
    for (let i = 0; i < lessons.length; i++) {
      ...
      query_1.first().then(async result => {
        ...
        this.resultLesson = this.oldLesson.concat(this.Lesson)
      });
    }
  }
