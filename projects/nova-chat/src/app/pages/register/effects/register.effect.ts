...
export class RegisterEffect {
    ...
    // 注册
    private register$: Observable<Action> = createEffect(() => this.actions$
    .pipe(
    ...
    switchMap(async (val) => {
            ...
            let data: any = await this.apiService.register(registerObj);
            if (data.code) {
                let usernameTip = '';
                if (data.code === 882002) {
                    usernameTip = '用户名已存在';
    ...
    // 用户名是否符合正则
    private isUsernameAvailable$: Observable<Action> = createEffect(() => this.actions$
        .pipe(
