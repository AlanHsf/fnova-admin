...
export class ChatPanelComponent implements OnInit, AfterViewInit, OnChanges, OnDestroy {
    ...
    @ViewChild('chatPanelContainer')  chatPanelContainer;
    @Input('messageList')
    ...
    @Input()
     selfInfo;
    @Input()
     otherOptionScrollBottom;
    @Input()
     changeActiveScrollBottom;
    @Output()
    ...
    @Output()
     sendLocation: EventEmitter<any> = new EventEmitter();
    @Output()
    ...
    get msgFile(){
        return JSON.parse(JSON.stringify(this._msgFile))
    }
    set msgFile(v){
        this._msgFile = JSON.parse(JSON.stringify(v))
    }
     businessCard = {
        show: false,
        info: []
    ...
    public ngOnDestroy() {
     ...
     scrollBottom(timeout, changeActive?) {
        if (changeActive) {
            this.loadFlag = false;
        }
        setTimeout(() => {
    ...
    @HostListener('document:dragenter', ['$event'])  onDragenter(event) {
        event.preventDefault();
    }
    ...
    @HostListener('window:keyup', ['$event'])  onKeyupWindow(event) {
     ...
     stateChanged(chatState, contactState) {
    ...
    // 点击更多列表的元素
     ...
     showYouMoreText(event, item, more) {
    ...
    // 发送文本
     sendMsgAction() {
        ...
        if (draft) {
            ...
            if(this.messageList[this.active.activeIndex].draft){ // 当存在draft时，再清空为"""
    ...
    // 输入框keyup
     ...
     filterNewList1(letter, newList, memberList) {
     ...
     filterNewList2(letter, newList, memberList) {
        for (let item of memberList) {
            if (item.memo_name &&
                item.memo_name.toUpperCase().indexOf(letter) !== -1) {
                item.match = 'memo_name';
    ...
    // 预览聊天文件中的图片
     showMsgFileImageViewerEmit(item) {
