...
export class DetailComponent implements OnInit {
  ...
  async getLesson() {
    ...
    this.activatedRoute.paramMap.subscribe(async param => {
      ...
      let result = await queryLesson.first().catch(err=>{
  ...
  getProgress() {
    let serverURL = 'https://server.fmode.cn/api/self-study/'
    ...
    querySurvey.find().then(res => {
      if(res[0] && res[0].get('schedule')){
        this.schedule = res[0].get('schedule')
      }      
      console.log(res[0].get('schedule'))
    })
    if(this.myProfile){
